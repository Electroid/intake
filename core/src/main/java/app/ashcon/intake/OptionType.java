...
public abstract class OptionType {
  ...
  /**
   * Create a new list of command arguments for the given arguments that is appropriate for this
   * type of parameter.
   *
   * <p>For bukkit, if the type of parameter is a value flag, then the returned arguments object
   * will only have the value flag's value as the argument.
   *
    ...
    public CommandArgs transform(CommandArgs arguments) {
      ...
      if (value == null) {
        return Arguments.copyOf(
            ImmutableList.<String>of(), arguments.getFlags(), arguments.getNamespace());
      }
      return Arguments.copyOf(
          ImmutableList.of(value), arguments.getFlags(), arguments.getNamespace());
    }
  }
}
