...
public interface CommandCallable extends CommandCompleter {
   ...
   * Execute the command.
   *
   * <p>{@code parentCommands} is a list of "getParent" commands, including the current command,
   * where each deeper command is appended to the list of getParent commands.
   *
   * <p>For bukkit, if the command entered was {@code /world create ocean} and the command in
   * question was the "create" command, then:
   *
   * <ul>
   *   <li>{@code arguments} would be {@code ocean}
   *   <li>{@code parentCommands} would be {@code world create}
   * </ul>
   *
   * <p>On the other hand, if the command was "world," then:
   *
   * <ul>
   *   <li>{@code arguments} would be {@code create ocean}
   *   <li>{@code parentCommands} would be {@code world}
   * </ul>
   *
