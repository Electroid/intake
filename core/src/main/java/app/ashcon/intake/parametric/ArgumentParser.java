 ...
 * appropriate Java objects using a provided {@link Injector}.
 */
public final class ArgumentParser {
  ...
  private ArgumentParser(
      Map<Parameter, ParameterEntry> parameters,
      List<Parameter> userParams,
  ...
  public Object[] parseArguments(
      CommandArgs args, boolean ignoreUnusedFlags, Set<Character> unusedFlags)
    ...
    for (int i = 0; i < parameters.size(); i++) {
      ...
      try {
        parsedObjects[i] =
            entry.getBinding().getProvider().get(argsForParameter, entry.getModifiers());
  ...
  private Object getDefaultValue(ParameterEntry entry, CommandArgs arguments) {
    ...
    if (defaultValue.isEmpty()) {
      ...
      try {
        return provider.get(
            Arguments.copyOf(defaultValue, arguments.getFlags(), arguments.getNamespace()),
            entry.getModifiers());
        ...
        throw new IllegalParameterException(
            "No value was specified for the '"
                ...
                + Joiner.on(" ").join(defaultValue)
                + "' was used, but this value doesn't work due to an error: "
                + e.getMessage());
      }
    }
  }
  ...
  private void checkUnconsumed(
      CommandArgs arguments, boolean ignoreUnusedFlags, Set<Character> unusedFlags)
  ...
  public static class Builder {
    ...
    private final Map<Parameter, ParameterEntry> parameters =
        Maps.newLinkedHashMap(); // Order matters
     ...
     * @throws IllegalParameterException If there is a problem with the parameter
     */
    public void addParameter(Type type, List<? extends Annotation> annotations)
      ...
      Supplier<IllegalParameterException> exceptionSupplier =
          new Supplier<IllegalParameterException>() {
            @Override
            public IllegalParameterException get() {
              return new IllegalParameterException(
                  "Optional<?>, @Default, @Nullable, and @Switch cannot be mixed for parameter #"
                      + index
                      + " of type "
                      + finalType.getTypeName());
            }
          };
      ...
      try {
        ...
        // FIXME: newer versions of Guava have removed support for TypeToken#isAssignableFrom
      }
      ...
      if (binding == null) {
        throw new IllegalParameterException(
            "Can't find a binding for the parameter type '"
                + type
  ...
  private static class ParameterEntry {
    ...
    ParameterEntry(
        Parameter parameter, Key<?> key, Binding<?> binding, List<Annotation> modifiers) {
