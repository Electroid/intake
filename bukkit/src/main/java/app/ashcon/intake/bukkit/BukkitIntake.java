...
/**
 * Omnibus API that hooks into a {@link JavaPlugin} and allows implementors to register commands.
 */
public class BukkitIntake implements CommandExecutor, TabCompleter {
  ...
  static {
    Bukkit.getServer()
        .getHelpMap()
        .registerHelpTopicFactory(BukkitCommand.class, new BukkitHelpTopic.Factory());
  }
  ...
  public void register() {
    ...
    if (dispatcher instanceof Lockable) ((Lockable) dispatcher).lock();

    commands.addAll(
        dispatcher.getCommands().stream()
   ...
   *
   * <p>Does not clear commands from the CommandMap.
   */
  @Deprecated
